-- [[ NAXOR V10.0 - THE OFFICIAL MASTER CODE ]] --
...
-- [[ 5. الخط الفاصل النيوني ]] --
local Separator = Instance.new("Frame", Main)
Separator.Size = UDim2.new(1, 0, 0, 1); Separator.Position = UDim2.new(0, 0, 0, 65)
Separator.BackgroundColor3 = Color3.fromRGB(40, 40, 40); Separator.BorderSizePixel = 0; Separator.ZIndex = 12
...
-- [[ 7. محتوى صفحة HOME (البطاقة الذكية) ]] --
...
local DName = Instance.new("TextLabel", IDCard); DName.Text = LP.DisplayName; DName.Size = UDim2.new(0, 110, 0, 20); DName.Position = UDim2.new(0, 85, 0, 18); DName.BackgroundTransparency = 1; DName.TextColor3 = Color3.new(1, 1, 1); DName.TextSize = 12; DName.Font = "GothamBold"; DName.TextXAlignment = "Left"
local UName = Instance.new("TextLabel", IDCard); UName.Text = "@"..LP.Name; UName.Size = UDim2.new(0, 110, 0, 20); UName.Position = UDim2.new(0, 85, 0, 34); UName.BackgroundTransparency = 1; UName.TextColor3 = Color3.fromRGB(150, 150, 150); UName.TextSize = 10; UName.Font = "GothamMedium"; UName.TextXAlignment = "Left"

-- نظام الرتب (Status)
local UserStatus = "OWNER" -- تقدر تخليها USER لو تبي
local StatusColor = MainColor
local StatusTag = Instance.new("Frame", IDCard); StatusTag.Size = UDim2.new(0, 65, 0, 18); StatusTag.Position = UDim2.new(0, 85, 0, 56); StatusTag.BackgroundColor3 = Color3.fromRGB(20, 20, 20); Instance.new("UICorner", StatusTag).CornerRadius = UDim.new(0, 4)
local STStroke = Instance.new("UIStroke", StatusTag); STStroke.Color = StatusColor; STStroke.Thickness = 1
local STText = Instance.new("TextLabel", StatusTag); STText.Size = UDim2.new(1, 0, 1, 0); STText.Text = UserStatus; STText.TextColor3 = StatusColor; STText.TextSize = 8; STText.Font = "GothamBold"; STText.BackgroundTransparency = 1

local function AddInfo(t, y, c)
    local l = Instance.new("TextLabel", IDCard); l.Text = t; l.Size = UDim2.new(1, -30, 0, 20); l.Position = UDim2.new(0, 15, 0, y); l.BackgroundTransparency = 1; l.TextColor3 = c or Color3.fromRGB(200, 200, 200); l.TextSize = 9; l.Font = "GothamMedium"; l.TextXAlignment = "Left"
    return l
end
local Device = (UIS.TouchEnabled and not UIS.KeyboardEnabled) and "Mobile" or (UIS.GamepadEnabled and "Console" or "PC")
AddInfo("Device: " .. Device, 88); AddInfo("Account Age: " .. LP.AccountAge .. " Days", 106); AddInfo("User ID: " .. LP.UserId, 124)
local PlayTimeLabel = AddInfo("PlayTime: 00:00:00", 150, StatusColor); PlayTimeLabel.Font = "GothamBold"

-- [[ 8. الأزرار السفلية ومستطيل الإعلانات ]] --
...
local PicBox = Instance.new("Frame", P.Home); PicBox.Size = UDim2.new(1, -275, 0, 210); PicBox.Position = UDim2.new(0, 250, 0, 25); PicBox.BackgroundColor3 = Color3.fromRGB(12, 12, 12); Instance.new("UICorner", PicBox).CornerRadius = UDim.new(0, 10); Instance.new("UIStroke", PicBox).Color = Color3.fromRGB(40, 40, 40)

-- [[ 9. نظام الوقت المستمر ]] --
local ST = os.time(); task.spawn(function() while task.wait(1) do local E = os.time()-ST; PlayTimeLabel.Text = string.format("PlayTime: %02d:%02d:%02d", math.floor(E/3600), math.floor((E%3600)/60), E%60) end end)
