-- [[ NAXOR OFFICIAL V75.5 - REAL USER TRACKING ]] --
...
local RealGlobalUsers = 1 -- يبدأ بك أنت كمستخدم أول
...
local OL = Instance.new("TextLabel", BB); OL.Size = UDim2.new(0, 220, 0, 20); OL.Position = UDim2.new(0, 15, 0, 12); OL.Text = "🌍 Global Users: 1"; OL.TextColor3 = Theme.Green; OL.Font = "GothamBold"; OL.TextSize = 10; OL.BackgroundTransparency = 1; OL.TextXAlignment = "Left"
...
-- [[ العداد المخصص لمستخدمي السكربت فقط ]] --
local UsersTable = {} -- مصفوفة لتخزين معرفات السيرفرات النشطة

task.spawn(function()
    pcall(function()
        MessagingService:SubscribeAsync("Naxor_User_Ping", function(message)
            local data = message.Data
            if data and data.id then
                UsersTable[data.id] = os.time() -- سجل وقت آخر إشارة من السيرفر
            end
        end)
    end)
end)
...
task.spawn(function()
    ...
    while task.wait(1) do
        -- إرسال "نبضة" كل 5 ثواني لتأكيد أنك مستخدم نشط
        if os.time() % 5 == 0 then
            pcall(function() 
                MessagingService:PublishAsync("Naxor_User_Ping", {id = game.JobId .. LP.UserId}) 
            end)
        end
        
        -- تنظيف القائمة (حذف النسخ اللي قفلت السكربت من أكثر من 15 ثانية)
        for id, lastSeen in pairs(UsersTable) do
            if os.time() - lastSeen > 15 then
                UsersTable[id] = nil
            end
        end
        
        -- حساب عدد المستخدمين الحقيقيين (أنت + البقية)
        local count = 0
        for _ in pairs(UsersTable) do count = count + 1 end
        RealGlobalUsers = math.max(1, count)
        
        -- تحديث الوقت والواجهة
        local e = os.time() - st; local h, m, s = math.floor(e/3600), math.floor((e%3600)/60), e%60
        SL.Text = string.format("🕒  Session: %02d:%02d:%02d", h, m, s)
        OL.Text = "🌍 Global Users: " .. RealGlobalUsers
    end
end)
